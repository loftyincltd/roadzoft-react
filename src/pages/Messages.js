...
function Messages() {
  ...
  const columns = [
    ...
    { selector: "created_at", name: "Date", sortable: true, cell: (row) => { 
      return <Moment fromNow>{row.created_at}</Moment>;
    }},
    { selector: "id", name: "", cell: (row) => {
      return (
        <ReadMessage title={row.title} body={row.body} />
      );
    },
  }
    ...
    <div>
      <div className="flex flex-row">
        ...
        <div className="dashboard-right">
          ...
          <h3 className="mx-5 mt-5 mb-3 font-bold text-gray-700 text-2xl">Inbox({messages.length})</h3>
          {hide == false && <NewMessage close={handleClose} onClick={() => setHide(false)} />}
          </div>
         
          <div className="main-items mx-5">
              <div>
                <ProjectTable data={messages} columns={columns} />
              </div>
